...
class AllPackagesets(APIWorker):
    ...
    def get_with_pkgs_count(self):
        response = self.send_sql_request(self.sql.get_all_pkgset_names_with_pkg_count)
        if not self.sql_status:
            return self.error
        if not response:
            return self.store_error(
    ...
    def get_summary(self):
        response = self.send_sql_request(
            self.sql.get_all_pkgsets_with_src_cnt_by_bin_archs
        )
        if not self.sql_status:
            return self.error
        if not response:
...
class PkgsetCategoriesCount(APIWorker):
    ...
    def get(self):
        ...
        response = self.send_sql_request(
            self.sql.get_pkgset_groups_count.format(branch=self.branch, sourcef=sourcef)
        )
        if not self.sql_status:
            return self.error
        if not response:
...
class AllPackagesetArchs(APIWorker):
    ...
    def get(self):
        ...
        response = self.send_sql_request(
            self.sql.get_all_bin_pkg_archs.format(branch=self.branch)
        )
        if not self.sql_status:
            return self.error
        if not response:
    ...
    def get_with_src_count(self):
        ...
        response = self.send_sql_request(
            self.sql.get_all_src_cnt_by_bin_archs.format(branch=self.branch)
        )
        if not self.sql_status:
            return self.error
        if not response:
