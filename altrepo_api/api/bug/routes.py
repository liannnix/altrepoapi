...
from .parsers import package_bugzilla_args, maintainer_bugzilla_args, bugzilla_by_edititon_args
...
class routeBugzillaByImageEdition(Resource):
    @ns.expect(bugzilla_by_edititon_args)
    @ns.marshal_list_with(bugzilla_info_model)
    def get(self):
        url_logging(logger, g.url)
        args = bugzilla_by_edititon_args.parse_args(strict=True)
        w = Bugzilla(g.connection, **args)
